...
import { ClipboardCheck, Square, AlertTriangle } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { stopClassroomLiveSession } from "@/services/api/teacher";

interface ExitTicketSessionControlsProps {
  classId: string;
...
/** Countdown for the published exit-ticket window (no timed practice before the quiz). */
export function ExitTicketSessionControls({
  classId,
  timeLimitMinutes,
  windowStartedAt,
  activeChapterId,
  onUpdate,
}: ExitTicketSessionControlsProps) {
  ...
  }, [windowStartedAt, timeLimitMinutes]);

  const handleStop = useCallback(async () => {
    try {
      await stopClassroomLiveSession(classId);
      toast.success("Exit ticket session ended.");
      onUpdate();
    } catch (err) {
      console.error(err);
      toast.error(
  ...
  const progressPercent =
    totalSeconds > 0 && secondsRemaining !== null ? (secondsRemaining / totalSeconds) * 100 : 100;
  ...
  return (
    <Card className={cn(isWarning && "border-warning/50", isExpired && "border-destructive/50")}>
      <CardHeader>
        ...
        <CardDescription>
          Students can open the exit ticket from practice. Timer tracks the quiz time limit for this session.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-foreground">
                {isExpired ? "Window ended" : isWarning ? "Almost done" : "Time remaining"}
              </span>
            </div>
            <span
              className={cn(
          ...
          <p className="text-xs text-muted-foreground mt-2">
            {isExpired
              ? "You can end the session for the class or leave it open for late submissions."
              : `${timeLimitMinutes} minute quiz window`}
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Badge variant="outline">{timeLimitMinutes} min limit</Badge>
