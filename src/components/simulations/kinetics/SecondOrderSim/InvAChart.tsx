/**
 * 1/[A] vs Time — straight line chart going UP with positive slope k.
 *
 * This is the diagnostic signature of second-order kinetics.
 * Note: The legacy app incorrectly labelled this as ln(A). This chart
 * deliberately fixes that scientific error — the Y-axis is 1/[A].
 */
import {
...
export function InvAChart({ series, tCurrent, halfLife, initialConc, reactantColor, reactantLabel }: Props) {
  const hl      = isFinite(halfLife) && halfLife <= MAX_TIME ? halfLife : null;
  const inv0    = 1 / initialConc;
  const invMax  = series.length ? series[series.length - 1].invA : inv0 + 10;
  const domain: [number, number] = [
    Math.floor(inv0 * 10) / 10,
    Math.ceil(invMax * 10) / 10,
  ];
      ...
      <div className="h-[160px] xl:flex-1 xl:min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={series} margin={{ top: 6, right: 10, bottom: 28, left: 4 }}>
            ...
            <Line type="linear" dataKey="invA" name={`1/[${reactantLabel}]`}
