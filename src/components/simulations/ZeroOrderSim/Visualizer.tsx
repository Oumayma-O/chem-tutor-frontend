 ...
 * Time-scrub control (◄ ▶ + play) lives here, directly below the chart.
 */
...
import type { DataPoint } from "./useKinetics";
    ...
    <div className="flex flex-col h-full gap-1.5">
      ...
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={series} margin={{ top: 6, right: 10, bottom: 28, left: -4 }}>
            ...
            <Line type="linear" dataKey="reactant" name={`[${reactantLabel}]`}
              stroke={reactantColor} strokeWidth={2.5} dot={false} activeDot={{ r: 3 }} isAnimationActive={false} />
            <Line type="linear" dataKey="product" name={`[${productLabel}]`}
      ...
      <div className="flex items-center justify-center gap-3 shrink-0 pb-1">
        ...
        <span className="text-sm tabular-nums min-w-[7rem] text-center">
          Time:{" "}
          <span className="font-semibold text-orange-500">{tCurrent.toFixed(1)} s</span>
        </span>
