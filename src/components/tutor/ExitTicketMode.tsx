...
import { Problem, SolutionStep, StudentAnswer } from "@/types/chemistry";
...
// Types for class-synced exit tickets
...
interface ExitTicketModeProps {
  // Legacy props for standalone mode
  problem?: Problem;
  timeLimit?: number;
...
export function ExitTicketMode({ problem, timeLimit: propTimeLimit, onComplete, onCancel, classId, configId }: ExitTicketModeProps) {
  ...
  const handleAnswerChange = (stepId: string, answer: string) => {
    setAnswers(prev => ({
      ...prev,
      [stepId]: { stepId, answer, isCorrect: undefined, attempts: prev[stepId]?.attempts || 0 },
    }));
  };
              ...
              <div key={q.id} className="bg-card border border-border rounded-lg p-5">
                ...
                {q.format === "qcm" && q.qcm_options && (
                  <RadioGroup
                    ...
                    onValueChange={(v) => handleClassAnswer(q.id, v)}
                    disabled={isComplete}
                  >
                    {q.qcm_options.map((opt, i) => (
                      <div key={i} className={cn(
                        ...
                        {isComplete && classAnswers[q.id] === opt.value && opt.value !== q.correct_answer && (
                ...
                {q.format === "structured" && (
                  <div className="space-y-4">
                    {/* Step 1: Equation ordering (if equation_parts available) */}
                    {q.equation_parts && q.equation_parts.length > 0 && (
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2 p-3 border border-dashed border-border rounded-lg min-h-[48px]">
                          {(() => {
                            // Shuffle equation parts for display
                    ...
                    {/* Step 2: Final numeric answer with units */}
                    <div>
                      ...
                      />
                      {q.unit && (
                        <p className="text-[10px] text-muted-foreground mt-1">Unit validation: {q.unit}</p>
                      )}
                    </div>
                  </div>
                )}
          ...
          <div className="mt-6">
            <Dialog open={showResults} onOpenChange={setShowResults}>
              <DialogContent className="max-w-md">
                ...
                {(() => {
                  let score: number, readyFlag: boolean, timeUsed: number;
                  if (isClassMode) {
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className={cn(
                        ...
                        readyFlag ? "bg-success/10 text-success" : "bg-yellow-500/10 text-yellow-600"
                      )}>
                        {readyFlag ? (
